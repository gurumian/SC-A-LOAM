...
class LaserPoseGraphOptNode : public rclcpp::Node
{
public:
    LaserPoseGraphOptNode(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : Node("laser_pgo", options)
    {
        this->declare_parameter<std::string>("save_directory", "/mnt/aloam/data/");

        this->get_parameter_or<std::string>("save_directory", save_directory, "/");
        ...
        this->get_parameter_or<double>("keyframe_meter_gap", keyframeMeterGap, 2.0);
        this->get_parameter_or<double>("keyframe_deg_gap", keyframeDegGap, 10.0);
            // // system params 
	...
	// nh.param<double>("sc_max_radius", scMaximumRadius, 80.0); // 80 is recommended for outdoor, and lower (ex, 20, 40) values are recommended for indoor 
        ...
        this->get_parameter_or<double>("sc_dist_thres", scDistThres, 0.2);
        this->get_parameter_or<double>("sc_max_radius", scMaximumRadius, 80.0);
        ...
        this->get_parameter_or<double>("mapviz_filter_size", mapVizFilterSize, 0.4);
        downSizeFilterMapPGO.setLeafSize(mapVizFilterSize, mapVizFilterSize, mapVizFilterSize);


        subLaserCloudFullRes_ = this->create_subscription<sensor_msgs::msg::PointCloud2>("/velodyne_cloud_registered_local", 100,
		std::bind(&LaserPoseGraphOptNode::laserCloudFullResHandler, this, std::placeholders::_1));
        subLaserOdometry_ = this->create_subscription<nav_msgs::msg::Odometry>("/aft_mapped_to_init", 100,
		std::bind(&LaserPoseGraphOptNode::laserOdometryHandler, this, std::placeholders::_1));
        subGPS_ = this->create_subscription<sensor_msgs::msg::NavSatFix>("/gps/fix", 100,
		std::bind(&LaserPoseGraphOptNode::gpsHandler, this, std::placeholders::_1));
        ...
        posegraph_slam_ = std::thread([&]() {
            process_pg();
        }); // pose graph construction
        lc_detection_ = std::thread ([&]() {
            process_lcd();
        }); // loop closure detection 
        icp_calculation_ = std::thread ([&]() {
            process_icp();
        }); // loop constraint calculation via icp 
        isam_update_ = std::thread ([&]() {
            process_isam();
        }); // if you want to call less isam2 run (for saving redundant computations and no real-time visulization is required), uncommment this and comment all the above runisam2opt when node is added. 

        viz_map_ = std::thread ([&](){
            process_viz_map();
        }); // visualization - map (low frequency because it is heavy)
        viz_path_ = std::thread ([&]() {
            process_viz_path();
    ...
    Pose6D getOdom(nav_msgs::msg::Odometry::ConstPtr _odom)
    {
        ...
        // Create a tf2::Matrix3x3 from the quaternion and get RPY values
        tf2::Matrix3x3(tf2_quat).getRPY(roll, pitch, yaw);

        // Assuming Pose6D is a custom data structure
        return Pose6D{tx, ty, tz, roll, pitch, yaw}; 
    } // getOdom
    ...
    void pubPath( void )
    {
        ...
        mKF.unlock(); 
        pubOdomAftPGO_->publish(odomAftPGO); // last pose 
        pubPathAftPGO_->publish(pathAftPGO); // poses 
        ...
        static tf2_ros::TransformBroadcaster br(this);  // 'node' is a shared pointer to the rclcpp::Node
        ...
        while(1)
        {
            while ( !scLoopICPBuf.empty() )
            {
                if( scLoopICPBuf.size() > 30 ) {
                    ...
                    RCLCPP_WARN(this->get_logger(), "Too many loop closure candidates to be ICPed is waiting ... Do process_lcd less frequently (adjust loopClosureFrequency)");
                }
...
private:
    ...
    std::thread posegraph_slam_;
    std::thread lc_detection_; 
    std::thread icp_calculation_; 
    std::thread isam_update_;
    std::thread viz_map_;
    std::thread viz_path_;
};
...
int main(int argc, char **argv)
{
    ...
    rclcpp::spin(std::make_shared<LaserPoseGraphOptNode>());
